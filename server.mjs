...
const watch = async () => {
    const watcher = fs.watch(path.join(__dirname, 'data'), { recursive: true });
...
const staticHandler = async (req, res) => {
    const filePath = path.normalize(path.join(__dirname, req.parsedUrl.pathname));
    try {
...
const longPollingHandler = (req, res) => {
    ...
    const lastSeendId = req.parsedUrl.searchParams.get('lastSeenId') || 0;
    // TODO: SELECT in DB
...
const sseHandler = (req, res) => {
    ...
    const lastSeendId = req.headers['last-event-id'] || 0;
    // TODO: SELECT in DB
    const newMessages = messages.filter(o => o.id > lastSeendId);
...
const wsHandler = (req, socket, head) => {
    ...
    socket.on('data', (data) => {
        ...
        console.log('parsed ws message', parsedMessage);

        if (parsedMessage.type === 'message') {
            const id = nextMessageId++;
            const newMessage = {
                id,
                data: parsedMessage.message || `message ${id}`,
            };
    ...
    // bad hack for upgrade
    req.parsedUrl = new URL(`http://localhost${req.url}`);

    const lastSeendId = req.parsedUrl.searchParams.get('lastSeenId') || 0;
    console.log('lastSeenId', lastSeendId);
    const newMessages = messages.filter(o => o.id > lastSeendId);
    if (newMessages.length > 0) {
        socket.sendJSON(newMessages);
    }
};
...
router.register('GET', '/api/path/{value}', (req, res) => {
    Router.json(req, res, { data: { value: req.params.groups.value } });
});
router.register('GET', '/api/test/form-defaults', (req, res) => {
    // req.parsedUrl.searchParams.get('key') or getAll('key')
    Router.json(req, res, { data: { value: 'ok' } });
});
router.register('POST', '/api/test/form-query', (req, res) => {
    // req.parsedUrl.searchParams.get('key') or getAll('key')
    Router.json(req, res, { data: { value: 'ok' } });
});
router.register('POST', '/api/test/form-urlencoded', (req, res) => {
    // req.body.get('key') or getAll('key')
    Router.json(req, res, { data: { value: 'ok' } });
}, formMiddleware);
router.register('POST', '/api/test/form-plain', (req, res) => {
    Router.json(req, res, { data: { value: 'ok' } });
});
router.register('POST', '/api/test/form-multipart', (req, res) => {
    // req.body.parts
    Router.json(req, res, { data: { value: 'ok' } });
}, multipartMiddleware);
...
router.register('POST', '/api/test/json', (req, res) => {
    // req.body
    Router.json(req, res, { data: { value: 'ok' } });
}, jsonMiddleware);
...
let nextMessageId = 1;
const messages = [
    { id: nextMessageId++, data: `message 1` },
    { id: nextMessageId++, data: `message 2` },
];

// GET/POST/other
// v1: return all messages
// v2: return all unseen messages: /api/test/poll?lastSeend=1
// plus: simple
// minus: low interactive
...
router.register('GET', '/api/test/message', (req, res) => {
    ...
    const newMessage = {
        id,
        data: req.parsedUrl.searchParams.get('data') || `message ${id}`,
    };
    messages.push(newMessage);

    longPollingClients.forEach((client) => client([newMessage]));

    sseClients.forEach((client) => client([newMessage]));
